...
static void interrupt (far *old_sb_isr)();
...
{
    unsigned char far *p;
    unsigned long phys1, phys1End;

    if (size == 0 || size > 32768U)
        return NULL;

    p = (unsigned char far *)_fmalloc((size_t)size * 2U);
    if (!p)
        return NULL;

    phys1    = ((unsigned long)FP_SEG(p) << 4) + (unsigned long)FP_OFF(p);
