...
#include <string.h>   /* _fmemset, _fmemcpy               */
...
/* Ligne horizontale directe dans le backbuffer.
   ...
   Appelle _fmemset directement, sans passer par drawLine. */
#define PUT_HLINE(xs, y, len, c) \
    _fmemset(backbuffer + OFFSET((xs),(y)), (c), (size_t)(len))
...
/* Rectangle plein : _fmemset ligne par ligne après clamp.
   Cas w==1 traité séparément pour éviter _fmemset de 1 octet
...
void drawRectFill(int x1, int y1, int x2, int y2, unsigned char color)
{
    ...
    if (len == 1)
    {
        /* Colonne verticale : PUT_PIXEL_UNSAFE évite _fmemset(…,1). */
        for (y = y1; y <= y2; y++)
...
/* =========================================================
   ...
   _fmemset directe). Élimine deux niveaux d'indirection par
