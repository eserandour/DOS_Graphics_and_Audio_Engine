/* =========================================================
   ...
   --------------------------------
   Pas de FPU en mode reel 8086. 1.0 = 65536L (FP_ONE).

   DECOUPAGE DE LA TEXTURE EN FAR HEAP
   ------------------------------------
   _fmalloc() prend un size_t 16 bits sous Watcom DOS
   (max 65535). Or 256*256 = 65536 depasse cette limite et
   deborde a 0 : l'allocateur recoit 0 et retourne NULL.
   On decoupe donc la texture en deux blocs de 128 lignes
   (128*256 = 32768 octets chacun) :
     tex0 = lignes   0..127
     tex1 = lignes 128..255
   L'acces s'effectue via la macro TEX_PIXEL(tx,ty).
...
/* =========================================================
   TABLE SINUS ET TEXTURE (far heap)
   =========================================================
   _fmalloc prend un size_t 16 bits sous Watcom DOS :
   valeur max = 65535. Or 256*256 = 65536 deborde a 0.
   On decoupe la texture en deux blocs de 128 lignes
   (128*256 = 32768 octets chacun) dans la limite des 16 bits.
   tex0 = lignes   0..127
   tex1 = lignes 128..255
   ========================================================= */
static long far          *sin_tab = NULL;
static unsigned char far *tex0    = NULL;
static unsigned char far *tex1    = NULL;
...
/* =========================================================
   ...
   =========================================================
   Lecture ligne par ligne (TEX_W octets par appel) pour
   rester dans les limites de fread() en 16 bits.
...
{
    if (tex0)    { _ffree(tex0);    tex0    = NULL; }
    if (tex1)    { _ffree(tex1);    tex1    = NULL; }
    if (sin_tab) { _ffree(sin_tab); sin_tab = NULL; }
}
...
void scene6(void)
{
    ...
    if (!initialized)
    {
        ...
        tex0 = (unsigned char far *)_fmalloc(TEX_HALF);
