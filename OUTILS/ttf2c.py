...
#   python3 ttf2c.py POLICE.ttf -o mafonte              → base de nom de sortie
...
#
# Sortie :
#   <nom>_WxH.c   → bloc prêt à coller dans font1dat.c
...
def generate_c_block(face, char_range, glyph_w: int, glyph_h: int,
    ...
    """
    Génère le corps de la fonction _initFont1_WxH() prêt à coller dans font1dat.c.
...
def process(ttf_path: str, glyph_w: int, glyph_h: int,
    ...
    c_file  = f"{out_base}_{glyph_w}x{glyph_h}.c"
    png_file = f"{out_base}_{glyph_w}x{glyph_h}.png"
    ...
    print()
    print("Intégration dans font1dat.c :")
    print(f"  1. Copier le contenu de {c_file} (le corps de _initFont1_{glyph_w}x{glyph_h})")
    print(f"     et le coller dans font1dat.c à la place de la section existante.")
    print(f"  2. Vérifier que _initFont1_{glyph_w}x{glyph_h}() est déclarée dans font1dat.h.")
...
def collect_files(args_list: list) -> list:
    ...
    Accepte un mélange de :
      ...
      - globs (*.ttf déjà expandés par le shell, ou pas)
    ...
    for arg in args_list:
        if os.path.isdir(arg):
            ...
            if os.path.isfile(arg):
                ...
                # Peut être un glob non expandé par le shell (Windows)
...
def main():
    parser = argparse.ArgumentParser(
        description="Convertit un ou plusieurs TTF en données C + PNG pour font1dat.c (style psf2c.py).",
...
"""
    ...
    parser.add_argument("-o", "--output", default=None,
        metavar="BASE",
        help="Base du nom de fichier de sortie (sans extension ni _WxH). "
             "Utilisable uniquement avec un seul fichier TTF en entrée.")
    ...
    # --- Vérification -o (interdit avec plusieurs fichiers) ---
    if args.output and len(files) > 1:
        print("Erreur : -o/--output ne peut être utilisé qu'avec un seul fichier TTF.",
              file=sys.stderr)
        sys.exit(1)
    ...
    for ttf_path in files:
        ...
        out_base = args.output if (args.output and len(files) == 1) else ttf_base
