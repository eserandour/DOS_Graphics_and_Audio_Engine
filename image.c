...
/* ---------------------------------------------------------
   ...
   fread ne peut pas ecrire directement dans un pointeur far
   avec certains runtimes DOS. On lit d'abord dans un buffer
   near (sur la pile), puis _fmemcpy transfere vers le far.
...
static int rawBlit(FILE *f, int srcW, int srcH, int dstX, int dstY)
{
    ...
    {
        /* Lire une ligne du .raw dans le buffer near. */
        if (fread(buf, 1, (size_t)srcW, f) != (size_t)srcW)
            return IMG_ERR_READ;

        /* Copier la ligne vers le backbuffer (far). */
...
/* ---------------------------------------------------------
   ...
   Ordre des operations :
     ...
     2. fopen + rawBlit -- lit le .raw ligne par ligne et
        copie chaque ligne dans le backbuffer via _fmemcpy.
     3. fclose.
